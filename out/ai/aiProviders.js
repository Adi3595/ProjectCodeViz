...
async function callOpenAI(config, messages) {
    ...
    const model = config.model || exports.PROVIDER_META.openai.defaultModel;
...
async function callGemini(config, messages) {
    const model = config.model || exports.PROVIDER_META.gemini.defaultModel;
...
async function callAnthropic(config, messages) {
    const model = config.model || exports.PROVIDER_META.anthropic.defaultModel;
...
async function callMistral(config, messages) {
    const model = config.model || exports.PROVIDER_META.mistral.defaultModel;
    const body = JSON.stringify({
        model,
        messages: messages.map(m => ({ role: m.role, content: m.content })),
        temperature: 0.3,
...
async function callCohere(config, messages) {
    const model = config.model || exports.PROVIDER_META.cohere.defaultModel;
    const systemMsg = messages.find(m => m.role === 'system')?.content;
...
async function callGroq(config, messages) {
    const model = config.model || exports.PROVIDER_META.groq.defaultModel;
    const body = JSON.stringify({
        model,
        messages: messages.map(m => ({ role: m.role, content: m.content })),
        temperature: 0.3,
