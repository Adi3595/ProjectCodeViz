        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (currentClass) {
                ...
                if (mMatch) {
                    ...
                    if (['if', 'for', 'while', 'switch', 'catch', 'return'].includes(mName)) {
                        continue;
                    }
            ...
            if (!currentClass) {
                ...
                if (vMatch && !arrowMatch) {
                    ...
                    if (['if', 'else', 'for', 'while', 'return'].includes(vName)) {
                        continue;
                    }
            ...
            while ((cm = callRx.exec(trimmed)) !== null) {
                const cn = cm[1];
                if (['if', 'for', 'while', 'switch', 'return', 'catch', 'new', 'require', 'import', 'console', 'setTimeout', 'setInterval', 'Promise', 'JSON', 'Object', 'Array', 'Math', 'Date', 'RegExp', 'Error', 'Map', 'Set', 'parseInt', 'parseFloat', 'isNaN', 'typeof', 'super'].includes(cn)) {
                    continue;
                }
                if (funcNames.has(cn)) {
                    const parentId = currentClass ? currentClass.id : fileId;
