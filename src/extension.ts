...
import { WorkspaceScanner } from './parser/workspaceScanner';
import { MultiLanguageParser } from './parser/multiLanguageParser';
import { RelationshipAnalyzer } from './analyzer/relationshipAnalyzer';
import { ErrorDetector } from './analyzer/errorDetector';
import { GraphBuilder } from './graph/graphBuilder';
import { WebviewPanel } from './webview/webviewPanel';
import { AIService } from './ai/aiService';
import { AIHoverProvider } from './ai/aiHoverProvider';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const aiHoverProvider = new AIHoverProvider(aiService);
  ...
  for (const lang of supportedLanguages) {
    context.subscriptions.push(
      vscode.languages.registerHoverProvider({ language: lang, scheme: 'file' }, aiHoverProvider)
    );
  }
    ...
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        try {
          ...
          const maxFileSizeKB: number = config.get('maxFileSizeKB') || 500;

          const scanner = new WorkspaceScanner(rootPath, ignorePaths, maxFileSizeKB);
          ...
          // Detect languages
          const langCounts = MultiLanguageParser.detectLanguages(files);
          ...
          const clangPath: string = config.get('clangPath') || 'clang';
          const errorDetector = new ErrorDetector(clangPath);
          ...
          WebviewPanel.createOrShow(context, graphData, rootPath);
  ...
  const explainCodeCmd = vscode.commands.registerCommand('projectcodewiz.explainCode', async () => {
    if (!ensureAIConfigured(aiService)) { return; }
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'ProjectCodeWiz AI: Explaining code...', cancellable: false },
      async () => {
        try {
          const result = await aiService.explainCode(code, language, fileName);
          AIResultPanel.show(context, `Code Explanation — ${fileName}`, result, aiService.getProviderLabel(), '📖');
        } catch (err: any) {
  ...
  const analyzeErrorsCmd = vscode.commands.registerCommand('projectcodewiz.analyzeErrors', async () => {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'ProjectCodeWiz AI: Analyzing errors...', cancellable: false },
      async () => {
        try {
          const result = await aiService.analyzeErrors(code, language, errorMessages, fileName);
          AIResultPanel.show(context, `Error Analysis — ${fileName}`, result, aiService.getProviderLabel(), '🔍');
        } catch (err: any) {
          vscode.window.showErrorMessage(`ProjectCodeWiz AI Error: ${err.message}`);
        }
      }
  ...
  const suggestImprovementsCmd = vscode.commands.registerCommand('projectcodewiz.suggestImprovements', async () => {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'ProjectCodeWiz AI: Suggesting improvements...', cancellable: false },
      async () => {
        try {
          const result = await aiService.suggestImprovements(code, language, fileName);
          AIResultPanel.show(context, `Improvement Suggestions — ${fileName}`, result, aiService.getProviderLabel(), '🚀');
        } catch (err: any) {
          vscode.window.showErrorMessage(`ProjectCodeWiz AI Error: ${err.message}`);
        }
      }
  ...
  const codeInsightsCmd = vscode.commands.registerCommand('projectcodewiz.codeInsights', async () => {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'ProjectCodeWiz AI: Generating insights...', cancellable: false },
      async () => {
        try {
          const result = await aiService.getCodeInsights(code, language, fileName);
          AIResultPanel.show(context, `Code Insights — ${fileName}`, result, aiService.getProviderLabel(), '📊');
        } catch (err: any) {
          vscode.window.showErrorMessage(`ProjectCodeWiz AI Error: ${err.message}`);
        }
      }
...
function updateAIStatusBar(statusBar: vscode.StatusBarItem, aiService: AIService) {
...
function ensureAIConfigured(aiService: AIService): boolean {
  if (!aiService.isConfigured()) {
    vscode.window.showWarningMessage(
      'ProjectCodeWiz AI is not configured. Set up your API key first.',
      'Configure Now'
    ).then(choice => {
...
function mapLanguageId(vscodeLangId: string): string {
