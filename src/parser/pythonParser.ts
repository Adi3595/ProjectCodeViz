    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (indent === 0) {
        ...
        if (varMatch2 && !trimmed.startsWith('#') && !trimmed.startsWith('import') && !trimmed.startsWith('from')) {
          ...
          if (['if', 'else', 'elif', 'for', 'while', 'return', 'yield', 'raise', 'try', 'except', 'finally', 'with', 'assert', 'pass', 'break', 'continue', 'del', 'print'].includes(varName)) { continue; }
      ...
      while ((callMatch = callRegex.exec(trimmed)) !== null) {
        ...
        if (['if', 'for', 'while', 'print', 'return', 'range', 'len', 'str', 'int', 'float', 'list', 'dict', 'set', 'tuple', 'type', 'isinstance', 'issubclass', 'super', 'enumerate', 'zip', 'map', 'filter', 'sorted', 'reversed', 'any', 'all', 'min', 'max', 'abs', 'sum', 'open', 'input', 'hasattr', 'getattr', 'setattr', 'delattr', 'property', 'staticmethod', 'classmethod'].includes(calledName)) { continue; }
